...
export const LetterPaper = forwardRef<HTMLDivElement, { letter: Letter }>(
  function LetterPaper({ letter }, ref) {
    ...
    return (
      <div
        ref={ref}
        className="relative mx-auto w-full max-w-3xl overflow-hidden rounded-3xl border border-seam bg-paper p-8 text-paper-foreground shadow-xl sm:p-10 md:p-14"
      >
        {/* subtle deckled top edge */}
        <div className="mx-auto mb-8 h-px w-16 bg-seam" aria-hidden="true" />
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-7">
          ...
          >
            ...
            <p className="mt-1 font-hand text-3xl text-paper-foreground sm:text-4xl">
              {letter.sender}
            </p>
          </motion.div>
        </motion.div>

        <div className="mx-auto mt-10 h-px w-16 bg-seam" aria-hidden="true" />
      </div>
    )
  },
)
